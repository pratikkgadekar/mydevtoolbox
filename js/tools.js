/* js/tools.js - Complete Developer Utility Suite Catalog */
...
(function initDatabase() {
  ...
  const rawTools = [
    ...
    ['groundedness-checker', 'Context Groundedness Citation Checker', 'Check whether assistant claims match verifiable sentence quotes in retrieved text', c.ai, 'check-square', 'Citation'],
    ['llm-latency-budget', 'TTFT & Streaming Latency Calculator', 'Calculate Time-To-First-Token (TTFT) and token generation speed per second', c.ai, 'activity', 'Latency'],
    ['prompt-variable-clean', 'Unused Prompt Variable Detector', 'Highlight declared {{variables}} in prompt templates that lack input bindings', c.ai, 'alert-circle', 'Templates'],
    ['tool-call-log-parser', 'LLM Tool Execution Log Parser', 'Extract and format nested JSON arguments and tool call outputs from agent logs', c.ai, 'file-text', 'Debugging'],
    ['prefix-caching-calc', 'KV Cache Prefix Savings Calculator', 'Calculate discount savings on cached prompt tokens for Anthropic and OpenAI', c.ai, 'database', 'Cache'],
    ['context-window-fit', 'Context Window Overflow Verifier', 'Check if system prompt, retrieved RAG docs, and history fit safe boundaries', c.ai, 'box', 'Limits'],
    ['agent-scratchpad', 'ReAct Scratchpad Log Formatter', 'Format Thought, Action, Action Input, and Observation logs for ReAct agents', c.ai, 'book', 'ReAct'],
    ['pydantic-ai-gen', 'Pydantic Output Parser Generator', 'Generate typed schema models for LangChain and LlamaIndex structured outputs', c.ai, 'code-2', 'LangChain'],
    ...
    ['embedding-norm-calc', 'L2 Euclidean Vector Normalizer', 'Calculate vector magnitude and normalize floats to unit length ||v||=1', c.ai, 'trending-up', 'Vector'],
    ['json-markdown-strip', 'Markdown Code Block JSON Extractor', 'Strip leading ```json and trailing ``` markers from raw LLM responses', c.ai, 'code', 'JSON'],
    ['hallucination-word-count', 'Response Density Word Analyzer', 'Calculate lexical richness and unique token ratio in LLM generation samples', c.ai, 'bar-chart-2', 'Metrics'],
    ...
    ['sql-injection-fuzz-list', 'SQLi Fuzzing Payload Test Bed', 'Generate standard single-quote and boolean SQL injection probe strings for form security', c.testing, 'shield-alert', 'Sec QA'],
    ...
    ['mobile-viewport-matrix', 'Mobile Viewport CSS Resolution Matrix', 'Inspect screen widths and DPRs for modern iPhone, Pixel, and Galaxy device profiles', c.testing, 'smartphone', 'Viewports'],
    ...
    ['broken-link-status-filter', 'HTTP Status Error Code Classifier', 'Categorize lists of returned HTTP status codes into 2xx Success, 4xx Client, and 5xx Server', c.testing, 'filter', 'HTTP Logs'],
    ...
    ['robot-framework-keywords', 'Robot Framework Keyword Formatter', 'Format test cases into pipe-delimited or 4-space separated Robot Framework tables', c.testing, 'cpu', 'Robot'],
    ['api-schema-diff', 'REST API Payload Schema Diff Inspector', 'Detect breaking contract changes (missing keys, altered types) between API versions', c.testing, 'git-compare', 'Contract'],
    ['luhn-algorithm-verify', 'Luhn Checksum Formula Validator', 'Calculate the modulo-10 Luhn check digit for identity numbers and loyalty barcodes', c.testing, 'check', 'Modulo 10'],
    ['form-input-stress-text', 'Form Input Extreme Stress Characters', 'Generate multi-byte CJK, Arabic RTL, and mathematical symbols to test text encodings', c.testing, 'globe', 'UTF-8 QA'],
    ...
    ['qa-test-case-markdown', 'Markdown Test Case Template Generator', 'Generate structured test cases with Preconditions, Steps, Expected, and Actual results', c.testing, 'file-text', 'Test Case'],
    ['mock-user-agent-headers', 'Device User-Agent Spoofing Directory', 'Copy authentic User-Agent strings for Googlebot, iOS Safari, and Android Chrome', c.testing, 'monitor', 'User-Agent'],
    ...
    ['flaky-test-score-calc', 'Flaky Test Severity Score Calculator', 'Calculate flakiness frequency weights based on run history and retry pass counts', c.testing, 'alert-circle', 'Flaky QA'],
    ...
    ['traefik-router-gen', 'Traefik v2/v3 Dynamic Config Builder', 'Generate Traefik HTTP routers and middlewares for Docker container labels', c.ops, 'git-merge', 'Traefik'],
    ...
    ['ufw-firewall-cmd-gen', 'Ubuntu UFW Firewall Rule Builder', 'Generate ufw allow from <ip> to any port <port> firewall rules', c.ops, 'shield-check', 'UFW'],
    ...
    ['salary-to-hourly', 'Annual Salary to Hourly Wage Equivalent', 'Convert annual gross pay into weekly, daily, and hourly wage rates (based on 2080 hrs)', c.num, 'dollar-sign', 'Salary'],
    ...
    ['unit-price-comparator', 'Price-per-Unit Comparison Tool', 'Compare multi-pack grocery sizes to find the lowest price per ounce, gram, or liter', c.num, 'shopping-cart', 'Best Value'],
    ...
    ['network-info-inspector', 'Network Connection & Effective Speed Sizer', 'Inspect downlink speeds (Mbps) and effective network types (4g/wifi) via navigator.connection', c.med, 'wifi', 'Network'],
    ['geolocation-coord-finder', 'Browser Geolocation Lat/Long Inspector', 'Inspect GPS latitude, longitude, and accuracy radius via the HTML5 Geolocation API', c.med, 'map-pin', 'GPS'],
    ...
    ['image-aspect-crop-calc', 'Image Aspect Ratio Crop Dimension Sizer', 'Calculate exact pixel dimensions to crop images to 16:9, 4:3, or 1:1 without distortion', c.med, 'scissors', 'Crop'],
    ...
    ['svg-path-bounding-box', 'SVG Path Visualizer & Bounding Sizer', 'Paste raw <path d="..."> vector code to preview outlines and bounding box boxes', c.med, 'code', 'SVG Path'],
    ...
    ['pdf-page-rotator', 'Client-Side PDF Page Rotator', 'Rotate landscape and portrait PDF pages (90°, 180°, 270°) locally in browser memory', c.med, 'rotate-cw', 'pdf-lib'],
    ...
    ['audio-bpm-tap-tempo', 'Tap Tempo (BPM) Music Metronome', 'Tap any key along with a song beat to calculate musical Beats Per Minute (BPM)', c.med, 'activity', 'BPM'],
  ...
  rawTools.forEach(([id, name, desc, cat, icon, badge]) => {
