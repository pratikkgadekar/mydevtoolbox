/* js/tools-data.js - Exactly 500 Unique Utilities (10 Categories × 50 Tools) */
window.toolsDatabase = [
  ...
  { id: 'chunk-overlap-calc', name: 'RAG Chunk & Overlap Calculator', desc: 'Calculate optimal chunk size and overlap token percentage for vector DBs', cat: 'ai', icon: 'layers', badge: 'RAG DB' },
  ...
  { id: 'jsonl-fine-tune', name: 'JSONL Training Pair Validator', desc: 'Validate prompt-completion and chat-format JSONL lines for fine-tuning', cat: 'ai', icon: 'file-check', badge: 'JSONL' },
  ...
  { id: 'role-tag-formatter', name: 'ChatML Role Tag Formatter', desc: 'Format system, user, and assistant blocks into ChatML and Llama formats', cat: 'ai', icon: 'message-square', badge: 'ChatML' },
  ...
  { id: 'prompt-fuzzer', name: 'System Prompt Leaking Fuzzer', desc: 'Generate adversarial test prompts to evaluate system prompt boundaries', cat: 'ai', icon: 'shield-alert', badge: 'Red Team' },
  { id: 'token-cost-estimator', name: 'Batch Inference Cost Estimator', desc: 'Calculate batch processing costs across million-token tiers for models', cat: 'ai', icon: 'calculator', badge: 'Pricing' },
  ...
  { id: 'negative-prompt-gen', name: 'Diffusion Negative Prompt Formatter', desc: 'Format quality-enhancing negative prompts for Midjourney and Diffusion', cat: 'ai', icon: 'ban', badge: 'Diffusion' },
  { id: 'rag-rerank-scorer', name: 'RAG Re-ranking Score Normalizer', desc: 'Normalize Cross-Encoder and BM25 relevance scores to 0-1 ranges', cat: 'ai', icon: 'bar-chart', badge: 'Rerank' },
  { id: 'json-function-gen', name: 'OpenAI Function Call Schema Builder', desc: 'Build JSON schemas with strict property types for OpenAI tools', cat: 'ai', icon: 'file-json', badge: 'Functions' },
  ...
  { id: 'prompt-diff-tester', name: 'Prompt Version A/B Diff Inspector', desc: 'Visually compare character additions and semantic changes between prompts', cat: 'ai', icon: 'git-compare', badge: 'A/B Test' },
  { id: 'multimodal-token-calc', name: 'Vision Model Image Token Calculator', desc: 'Calculate token weights for images based on resolution tiles and patches', cat: 'ai', icon: 'image', badge: 'Vision' },
  { id: 'speech-token-calc', name: 'Audio Speech Token Duration Sizer', desc: 'Estimate Whisper token consumption and API transcription cost per minute', cat: 'ai', icon: 'mic', badge: 'Whisper' },
  ...
  { id: 'ai-persona-builder', name: 'Agent Persona Definition Formatter', desc: 'Structure agent role, constraints, tone, and knowledge boundaries', cat: 'ai', icon: 'user', badge: 'Agents' },
  { id: 'rag-triplet-builder', name: 'Knowledge Graph Triplet Extractor', desc: 'Format subject-predicate-object semantic triplets for graph stores', cat: 'ai', icon: 'share-2', badge: 'Graph RAG' },
  { id: 'synthetic-qa-gen', name: 'Synthetic Q&A Dataset Builder', desc: 'Structure multi-turn conversational evaluation pairs for benchmarks', cat: 'ai', icon: 'help-circle', badge: 'Eval' },
  { id: 'groundedness-checker', name: 'Context Groundedness Citation Checker', desc: 'Check whether claims match verifiable sentence quotes in retrieved text', cat: 'ai', icon: 'check-square', badge: 'Citation' },
  { id: 'llm-latency-budget', name: 'TTFT & Streaming Latency Calculator', desc: 'Calculate Time-To-First-Token (TTFT) and token generation speed per sec', cat: 'ai', icon: 'activity', badge: 'Latency' },
  { id: 'prompt-variable-clean', name: 'Unused Prompt Variable Detector', desc: 'Highlight declared {{variables}} in prompt templates lacking input bindings', cat: 'ai', icon: 'alert-circle', badge: 'Templates' },
  { id: 'tool-call-log-parser', name: 'LLM Tool Execution Log Parser', desc: 'Extract and format nested JSON arguments and tool call outputs from logs', cat: 'ai', icon: 'file-text', badge: 'Debugging' },
  { id: 'prefix-caching-calc', name: 'KV Cache Prefix Savings Calculator', desc: 'Calculate discount savings on cached prompt tokens for Anthropic/OpenAI', cat: 'ai', icon: 'database', badge: 'Cache' },
  { id: 'context-window-fit', name: 'Context Window Overflow Verifier', desc: 'Check if prompt + retrieved RAG docs + history fit safe boundaries', cat: 'ai', icon: 'box', badge: 'Limits' },
  { id: 'agent-scratchpad', name: 'ReAct Scratchpad Log Formatter', desc: 'Format Thought, Action, Action Input, and Observation logs for agents', cat: 'ai', icon: 'book', badge: 'ReAct' },
  { id: 'pydantic-ai-gen', name: 'Pydantic Output Parser Generator', desc: 'Generate typed schema models for LangChain and LlamaIndex outputs', cat: 'ai', icon: 'code-2', badge: 'LangChain' },
  ...
  { id: 'json-markdown-strip', name: 'Markdown Code Block JSON Extractor', desc: 'Strip leading ```json and trailing ``` markers from raw LLM outputs', cat: 'ai', icon: 'code', badge: 'JSON' },
  { id: 'hallucination-word-count', name: 'Response Density Word Analyzer', desc: 'Calculate lexical richness and unique token ratio in LLM responses', cat: 'ai', icon: 'bar-chart-2', badge: 'Metrics' },
  ...
  { id: 'multiturn-history-trim', name: 'Chat History Sliding Window Trimmer', desc: 'Trim oldest conversation turns while preserving foundational system prompt', cat: 'ai', icon: 'clock', badge: 'Chat' },
  ...
  { id: 'json-key-case', name: 'JSON Key Case Transformer', desc: 'Recursively convert object keys between camelCase, snake_case, and kebab', cat: 'data', icon: 'type', badge: 'Keys' },
  { id: 'csv-transpose', name: 'CSV Column & Row Transposer', desc: 'Invert spreadsheet matrix axes by swapping columns into rows and vice versa', cat: 'data', icon: 'refresh-ccw', badge: 'Pivot' },
  ...
  { id: 'json-filter-query', name: 'JSON Predicate Array Filter', desc: 'Filter arrays of JSON objects by key conditions (age > 21, status == true)', cat: 'data', icon: 'filter', badge: 'Filter' },
  ...
  { id: 'hex-dump-to-json', name: 'Hex Dump to Binary Byte Array', desc: 'Parse space-separated hex dumps into integer byte arrays', cat: 'data', icon: 'binary', badge: 'Hex' },
  { id: 'json-type-inference', name: 'JSON Schema Type Extractor', desc: 'Infer draft-07 JSON schemas from sample JSON payloads with enums', cat: 'data', icon: 'file-check', badge: 'Schema' },
  ...
  // =========================================================================
  // 3. SECURITY & CRYPTOGRAPHY (50 TOOLS)
  // =========================================================================
  ...
  { id: 'basic-auth-header', name: 'Basic Auth Header Generator', desc: 'Generate Base64 Authorization: Basic headers from username and password', cat: 'security', icon: 'shield-alert', badge: 'Headers' },
  ...
  { id: 'sha3-256-hasher', name: 'SHA3-256 (Keccak) Message Hasher', desc: 'Calculate sponge-construction SHA-3 cryptographic digests in browser', cat: 'security', icon: 'hash', badge: 'SHA-3' },
  ...
  { id: 'password-strength', name: 'Password Common Dictionary Fuzzer', desc: 'Evaluate passwords against top 10,000 common passwords and leaks', cat: 'security', icon: 'alert-triangle', badge: 'Strength' },
  { id: 'argon2-param-calc', name: 'Argon2id Memory Cost Sizer', desc: 'Calculate time, memory (KiB), and parallelism parameters for Argon2', cat: 'security', icon: 'sliders', badge: 'Argon2' },
  { id: 'file-hash-verify', name: 'File Checksum Integrity Matcher', desc: 'Drag-and-drop a file and compare its hash against expected signatures', cat: 'security', icon: 'check-circle', badge: 'Verify' },
  { id: 'base64url-codec', name: 'Base64URL Safe Converter', desc: 'Convert standard Base64 to URL-safe Base64 by substituting + with -', cat: 'security', icon: 'link', badge: 'RFC 7515' },
  { id: 'xor-cipher-tool', name: 'XOR Stream Cipher Sandbox', desc: 'Perform bitwise XOR encryption and decryption on text strings with keys', cat: 'security', icon: 'refresh-ccw', badge: 'XOR' },
  ...
  { id: 'public-key-fingerprint', name: 'Public Key SHA-256 Fingerprinter', desc: 'Derive base64 and hex colon-separated fingerprints from raw keys', cat: 'security', icon: 'fingerprint', badge: 'Key ID' },
  ...
  { id: 'ssl-expiry-countdown', name: 'SSL Certificate Expiry Calculator', desc: 'Calculate exact days remaining before SSL/TLS certificates expire', cat: 'security', icon: 'calendar', badge: 'SSL' },
  ...
  { id: 'cookie-header-fuzzer', name: 'Session Token Masker & Fuzzer', desc: 'Inspect session tokens to check length, character randomness, and entropy', cat: 'security', icon: 'shield', badge: 'Session' },
  ...
  { id: 'html-script-sanitizer', name: 'XSS String Payload Neutralizer', desc: 'Neutralize JavaScript event handlers and script injection strings', cat: 'security', icon: 'shield-alert', badge: 'Anti-XSS' },
  { id: 'ssh-keygen-command', name: 'ssh-keygen Command Generator', desc: 'Generate commands for Ed25519 and RSA keys with custom comments', cat: 'security', icon: 'terminal', badge: 'SSH Gen' },
  { id: 'content-disposition-safe', name: 'Safe Filename Content-Disposition', desc: 'Sanitize download filenames to prevent directory traversal attacks', cat: 'security', icon: 'download', badge: 'Headers' },

  // =========================================================================
  // 4. WEB, APIS & NETWORKING (50 TOOLS)
  // =========================================================================
  ...
  // =========================================================================
  // 7. CONTENT, TEXT & MARKDOWN (50 TOOLS)
  // =========================================================================
  ...
  // =========================================================================
  // 8. MATH, TIME & FINANCIAL (50 TOOLS)
  // =========================================================================
  ...
  { id: 'simple-interest-calc', name: 'Simple Interest & Payoff Calculator', desc: 'Calculate basic non-compounding interest (I = P * r * t) on loans', cat: 'math', icon: 'dollar-sign', badge: 'Interest' },
  ...
  { id: 'std-dev-calc', name: 'Standard Deviation & Variance Calculator', desc: 'Calculate population and sample variance and standard deviation', cat: 'math', icon: 'activity', badge: 'Variance' },
  ...
  { id: 'salary-to-hourly', name: 'Annual Salary to Hourly Wage Equivalent', desc: 'Convert annual gross pay into weekly, daily, and hourly wage rates (2080 hrs)', cat: 'math', icon: 'dollar-sign', badge: 'Salary' },
  ...
  { id: 'unit-price-comparator', name: 'Price-per-Unit Comparison Tool', desc: 'Compare multi-pack grocery sizes to find the lowest price per ounce or liter', cat: 'math', icon: 'shopping-cart', badge: 'Best Value' },
  ...
  // =========================================================================
  // 9. QA AUTOMATION, LOCATORS & MOCKS (50 TOOLS)
  // =========================================================================
  ...
  // =========================================================================
  // 10. FILE, IMAGE & MEDIA UTILITIES (50 TOOLS)
  // =========================================================================
  ...
  { id: 'svg-path-bounding-box', name: 'SVG Path Visualizer & Bounding Sizer', desc: 'Paste raw path vector code to preview outlines and bounding boxes', cat: 'media', icon: 'code', badge: 'SVG Path' },
  ...
  { id: 'pdf-page-rotator', name: 'Client-Side PDF Page Rotator', desc: 'Rotate landscape and portrait PDF pages (90, 180, 270 deg) locally in browser memory', cat: 'media', icon: 'rotate-cw', badge: 'pdf-lib' },
